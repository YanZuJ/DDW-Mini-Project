// Transcrypt'ed from Python, 2023-09-22 15:35:30
var random = {};
	...
	if (value == '') {
		...
		for (var i = 0; i < len (array_str); i++) {
			if (array_str [i] == '') {
				...
				array_ls.append (array_str [i]);
			}
		}
		var insertion_sort = function (ls) {
			var n = len (ls);
			for (var out_idx = 1; out_idx < n; out_idx++) {
				var inn_idx = out_idx;
				var temp = ls [inn_idx];
				while (inn_idx > 0 && ls [inn_idx - 1] > temp) {
					var __left0__ = tuple ([ls [inn_idx], ls [inn_idx - 1]]);
					ls [inn_idx - 1] = __left0__ [0];
					ls [inn_idx] = __left0__ [1];
					inn_idx--;
				}
			}
			var strg = ', ';
			var strg = strg.join ((function () {
				var __accu0__ = [];
				for (var i of ls) {
		...
		var array_str = insertion_sort (array_ls);
	}
	document.getElementById ('sorted').innerHTML = array_str;
};
